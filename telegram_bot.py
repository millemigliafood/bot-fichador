...
# --- FUNCIONES DE UTILIDAD (Lectura y Escritura de JSON) ---
...
def leer_json(ruta, por_defecto):
    try:
        ...
        backups = sorted([f for f in os.listdir('datos') if f.startswith(os.path.basename(ruta) + ".bak")], reverse=True)
        if backups:
            print(f"Archivo JSON corrupto o no encontrado. Recuperando desde {backups[0]}")
            try:
                with open(os.path.join('datos', backups[0]), 'r', encoding='utf-8') as f:
                    return json.load(f)
            except Exception as e:
                print(f"Fallo al recuperar backup: {e}")
        return por_defecto
...
def guardar_json(ruta, datos):
    try:
        ...
        if os.path.exists(ruta):
            backup_path = os.path.join("datos", os.path.basename(ruta) + ".bak." + datetime.now().strftime("%Y%m%d%H%M%S"))
...
def validar_usuario(user_id):
    return leer_json(RUTA_EMPLEADOS, {}).get(str(user_id))
...
async def mensaje_general(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Lógica de botones principales (si no hay una acción en curso) ---
    ...
    # --- Lógica de conversaciones (si hay una acción en curso) ---

    if texto == "❌ Cancelar":
        return await limpiar_y_mostrar_menu(context, update, empleado)

    # --- Flujo: Ver Calendario Laboral (Empleado) ---
    ...
    # --- Flujo: Resumen Personal por Fechas (Todos los usuarios) ---
    ...
    # --- Flujo: Ver Fichajes Detallados (Admin) ---
    ...
    if siguiente_accion == "planificar_pedir_datos":
        try:
            ...
            calendario[clave_mes][uid].update(turnos_nuevos) # Actualiza o añade los nuevos turnos
...
# --- FUNCIONES DE VISUALIZACIÓN Y LÓGICA ---

def parse_mes_anio(texto: str) -> tuple[int | None, int | None]:
    """Parsea un texto para extraer mes y año."""
...
async def generar_imagen_calendario(uid: str, nombre_empleado: str, anio: int, mes: int) -> str | None:
    """Genera una imagen PNG del calendario de un empleado para un mes y año dados."""
    ...
    ruta_salida = os.path.join("datos", f"calendario_{uid}_{anio}_{mes}.png")
    try:
        # Añadimos opciones para mejorar la calidad de la imagen
...
def main():
    ...
    if not os.path.exists(RUTA_EMPLEADOS): guardar_json(RUTA_EMPLEADOS, {str(ADMIN_ID): {"nombre": "Administrador", "es_admin": True}})
    if not os.path.exists(RUTA_FICHAJES): guardar_json(RUTA_FICHAJES, [])
    if not os.path.exists(RUTA_CALENDARIO): guardar_json(RUTA_CALENDARIO, {})
